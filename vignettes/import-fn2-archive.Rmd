...
library(dplyr)
library(purrr)
gn <- map(gfsgn, ~filter(.x, PRJ_CD == "LOA_IA00_GL1"))
```
...
```{r import, eval=FALSE}
gn <- import_fn_index_net("FNData/GL1/IA00_GL1/DATA.ZIP")
```
...
list2env(gn, envir = .GlobalEnv) # to move list elements to global env.
